...
namespace Games {
    public class SokobanPlayer : AiAgent, IStateDelegate {
        ...
        public override void Move(Vector2 direction) {
            ...
            if (Physics.Raycast(destination - transform.forward * 5, transform.forward, out var hit, 10)) {
                ...
                if (crateMask == (crateMask | (1 << hit.collider.gameObject.layer))) {
                    ...
                    if (Physics.Raycast(crateDestination - transform.forward * 5, transform.forward, out var hit2,
                        10)) {
        ...
        private List<int[][]> GenerateAllPossibleStates(List<int[][]> previousGrids, int remainingcrates, int gridWidth,
            int gridHeight) {
            //remaining crates 1
        ...
        public int GetReward(GridState currentState, Movement action, List<GridState> possibleStates,
            out GridState nextState) {
            ...
            var crates = nextState.objectsPositions.FindAll(v => v.Key == layerCrate);
            var arrivals = nextState.objectsPositions.FindAll(v => v.Key == layerArrival);
            for (int i = crates.Count - 1; i >= 0; --i) {
                if (arrivals.Exists(arrival => arrival.Value == crates[i].Value))
        ...
        public GridState GetNextState(GridState currentState, Movement action, List<GridState> possibleStates,
            out int playerNewI,
            out int playerNewJ) {
            var nextState = new GridState(currentState);
            playerNewI = -1;
            playerNewJ = -1;
            Vector2Int gridSize = new Vector2Int(GameManager.Instance.startGrid[0].Length,
                GameManager.Instance.startGrid.Length);

            var playerObj = currentState.objectsPositions.Find(v => v.Key == layerPlayer);
            int playerDestination;

            int i = playerObj.Value.x;
            int j = playerObj.Value.y;
